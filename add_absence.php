...
// جلب بيانات الطالب
$stmt = $conn->prepare("
    SELECT s.*, l.id as level_id, l.name as level_name, 
           d.id as department_id, d.name as department_name,
           sd.id as sub_department_id, sd.name as sub_department_name
    FROM students s
...
// جلب المواد المتاحة للطالب بناءً على مستواه الدراسي
$stmt = $conn->prepare("
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($subject_id) || empty($date)) {
        ...
        // إدخال الغياب في قاعدة البيانات
        $stmt = $conn->prepare("
            INSERT INTO absences (student_id, subject_id, date, status, reason, justification_document)
            VALUES (?, ?, ?, ?, ?, ?)
        ");
        $stmt->bind_param("iissss", $student['id'], $subject_id, $date, $status, $reason, $justification_document);
        
        if ($stmt->execute()) {
            // إرسال إشعار إلى الأدمن المناسب
            $admin_notification = "Nouvelle absence à justifier: " . $student['first_name'] . ' ' . $student['last_name'];
            $stmt = $conn->prepare("
                INSERT INTO notifications (user_id, message)
                SELECT u.id, ?
                FROM users u
                WHERE u.role = 'admin' AND u.department_id = ?
            ");
            $stmt->bind_param("si", $admin_notification, $student['department_id']);
            $stmt->execute();
            
            $_SESSION['success'] = "Absence déclarée avec succès!";
...
<head>
    ...
    <title>Déclarer une absence - Justifly</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;900&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        /* نفس أنماط CSS من لوحة التحكم */
        :root {
            --bg-primary: #0f172a;
            --bg-secondary: #1e293b;
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-3xl font-bold mb-2">Déclarer une absence</h1>
                <p class="text-gray-400">Remplissez le formulaire ci-dessous pour déclarer votre absence</p>
            </div>
        ...
        <div class="form-container">
            ...
            <form method="POST" enctype="multipart/form-data" id="absence-form">
                ...
                <div class="bg-gray-800/50 rounded-lg p-4 mb-6">
                    <h3 class="font-semibold mb-2">Informations de l'étudiant</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div>
                            <span class="text-gray-400">Nom:</span>
                            <span class="ml-2"><?php echo htmlspecialchars($student['first_name'] . ' ' . $student['last_name']); ?></span>
                        </div>
                        <div>
                            <span class="text-gray-400">Niveau:</span>
                            <span class="ml-2"><?php echo htmlspecialchars($student['level_name']); ?></span>
                        </div>
                        <div>
                            <span class="text-gray-400">Département:</span>
                            <span class="ml-2"><?php echo htmlspecialchars($student['department_name']); ?></span>
                        </div>
                        <div>
                            <span class="text-gray-400">Spécialité:</span>
                            <span class="ml-2"><?php echo htmlspecialchars($student['sub_department_name']); ?></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="date" class="form-input" id="date" name="date" required max="<?php echo date('Y-m-d'); ?>">
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-textarea" id="reason" name="reason" 
                              placeholder="Décrivez brièvement le motif de votre absence..."></textarea>
                </div>

                <!-- Justification Document -->
                <div class="form-group">
                    <label class="form-label">
                        <i class="fas fa-paperclip mr-2"></i>Document de justification (optionnel)
                    </label>
                    <div class="file-upload">
                        ...
                        <label for="justification_document" class="file-upload-label">
                            ...
                            <div>
                                <p class="font-semibold">Cliquez pour télécharger</p>
